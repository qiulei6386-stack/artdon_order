...
$paths = array_merge(
    ['', 'contact', 'lighting-simulation'],
    array_keys($routes),
    array_map(static fn(array $p): string => 'product/' . $p['sku'], $products)
);
$paths = array_values(array_unique($paths));
echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
echo "<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n";
foreach ($paths as $item) {
    if (str_starts_with($item, 'account') || $item === 'ai/lighting-simulation') continue;
