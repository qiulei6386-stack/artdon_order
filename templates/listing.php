...
<section class="section listing-section">
    <div class="container listing-layout">
        <aside class="filter-panel" data-filter-panel>
            ...
            <div class="filter-group"><h3>Demo availability fields</h3><label><input type="checkbox" data-list-filter="availability" value="ready"> Demo ready stock</label><label><input type="checkbox" data-list-filter="availability" value="new"> Demo new item</label><label><input type="checkbox" data-list-filter="availability" value="clearance"> Demo clearance</label></div>
        ...
        <div class="listing-main">
            <div class="listing-toolbar"><button class="button button-outline mobile-filter-button" type="button" data-filter-open><?= icon('filter') ?> Filters</button><div><strong><span data-visible-count><?= count($listProducts) ?></span> products</strong><?php if($q!==''): ?><span>for “<?= e($q) ?>”</span><?php endif; ?></div><label>Sort demo data<select data-product-sort><option value="recommended">Recommended</option><option value="stock-desc">Demo stock: high to low</option><option value="price-asc">Demo price: low to high</option><option value="newest">Demo newest</option></select></label></div>
            <?php if ($listProducts): ?>
